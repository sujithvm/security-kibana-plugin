        ...
        request.auth.securitySessionStorage = {
            ...
            authenticate: async function(credentials, options = {}) {
                try {
                    ...
                    server.log(['info', '***** opendistro-security'], "file:sessionPlugin func:authenticate obj:credentials " + JSON.stringify(credentials));
                    ...
                    server.log(['info', '***** opendistro-security'], "file:sessionPlugin func:authenticate obj:authResponse " + JSON.stringify(authResponse));
            ...
            _handleAuthResponse: function(credentials, authResponse, additionalAuthHeaders = {}) {
                server.log(['info', '***** opendistro-security'], "file:sessionPlugin func:_handleAuthResponse obj:authResponse " + JSON.stringify(authResponse));
                ...
                if (settings.validateAvailableRoles && (!authResponse.user.roles || authResponse.user.roles.length === 0)) {
                    server.log(['info', '***** opendistro-security'], "file:sessionPlugin func:_handleAuthResponse msg:missing role ");
