...
export default function (pluginRoot, server, APP_ROOT, API_ROOT) {
    ...
    server.auth.scheme('security_access_control_scheme', (server, options) => ({
        authenticate: (request, reply) => {
            ...
            server.log(['info', 'OpenDistro Security Hapi auth'], "authenticate ");
            ...
            if (request.headers.authorization) {
                ...
                server.log(['info', 'OpenDistro Security Hapi auth'], "authenticate credentials " + JSON.stringify(credentials));
            ...
            if (unauthenticatedRoutes.includes(request.path)) {
                ...
                server.log(['info', 'OpenDistro Security Hapi auth'], "authenticate unauthenticatedRoutes " + JSON.stringify(unauthenticatedRoutes));
                server.log(['info', 'OpenDistro Security Hapi auth'], "authenticate unauthenticatedRoutes " + JSON.stringify(credentials));
    ...
    server.ext('onPostAuth', function (request, next) {
        ...
        server.log(['info', 'OpenDistro Security Hapi auth'], "onPostAuth " + JSON.stringify(request));
