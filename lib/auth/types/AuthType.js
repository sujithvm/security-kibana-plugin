...
export default class AuthType {
    ...
    async init() {
        this.server.log(['info', 'OpenDistro Security AuthType'], "AuthType init ");
        this.setupStorage();
    ...
    setupStorage() {
        this.server.log(['info', 'OpenDistro Security AuthType'], "AuthType setupStorage ");
    ...
    setupAuthScheme() {
        this.server.log(['info', 'OpenDistro Security AuthType'], "AuthType setupAuthScheme ");
        this.server.auth.scheme('security_access_control_scheme', (server, options) => ({
            authenticate: async(request, h) => {
                let credentials = null;

                server.log(['info', 'OpenDistro Security AuthType'], "AuthType setupAuthScheme ");

                if (this.routesToIgnore.includes(request.path)) {
                    return h.continue;
                }

                server.log(['info', 'OpenDistro Security AuthType'], "AuthType unauthenticatedRoutes " + JSON.stringify(this.unauthenticatedRoutes));
                ...
                server.log(['info', 'OpenDistro Security AuthType'], "AuthType test " + request.path);

                try {
                    credentials = await this.server.auth.test('security_access_control_cookie', request);

                    server.log(['info', 'OpenDistro Security AuthType'], "AuthType test credentials " + JSON.stringify(credentials));

                    return h.authenticated({credentials})
                 } catch(error) {
                    //this.server.log(['info', 'OpenDistro Security AuthType'], "AuthType credentials " + JSON.stringify(error));
                    ...
                    server.log(['info', 'OpenDistro Security AuthType'], "AuthType test authHeaderCredentials " + JSON.stringify(authHeaderCredentials));
                    ...
                    if (authHeaderCredentials) {
                        try {
                            ...
                            server.log(['info', 'OpenDistro Security AuthType'], "AuthType test session " + JSON.stringify(session));
