...
export default class AuthType {
    ...
    constructor(pluginRoot, server, kbnServer, APP_ROOT, API_ROOT) {
         ...
         * @type {string[]}
         */
        this.allowedAdditionalAuthHeaders = ['security_impersonate_as', 'X-AMZN-AES-AUTH-CACHE-ID', 'X-AMZN-AES-AUTH-REQUIRED', 'authorization', 'securitytenant', 'security_tenant', 'cognitousername'];
    ...
    async init() {
        this.server.log(['info', '***** opendistro-security'], "file:AuthType func:init");
    ...
    setupStorage() {
        this.server.log(['info', '***** opendistro-security'], "file:AuthType func:setupStorage obj:type " + this.type);
    ...
    getAuthHeader(session) {
        this.server.log(['info', '***** opendistro-security'], "file:AuthType func:getAuthHeader");
    ...
    detectAuthHeaderCredentials(request, sessionCredentials = null) {
        this.server.log(['info', '***** opendistro-security'], "file:AuthType func:detectAuthHeaderCredentials");
        ...
        if (request.headers[this.authHeaderName]) {
            ...
            if (sessionCredentials !== null && sessionCredentials.authHeaderValue === authHeaderValue) {
                ...
                this.server.log(['info', '***** opendistro-security'], "file:AuthType func:detectAuthHeaderCredentials msg:return null");

                return null;
            }

            this.server.log(['info', '***** opendistro-security'], "file:AuthType func:detectAuthHeaderCredentials msg:return " + authHeaderValue);
        ...
        this.server.log(['info', '***** opendistro-security'], "file:AuthType func:init msg:return null");
    ...
    setupAuthScheme() {
        this.server.log(['info', '***** opendistro-security'], "file:AuthType func:setupAuthScheme");

        this.server.auth.scheme('security_access_control_scheme', (server, options) => ({
            authenticate: async(request, h) => {
                let credentials = null;

                server.log(['info', '***** opendistro-security'], "file:AuthType func:setupAuthScheme obj:path " + request.path);

                if (this.routesToIgnore.includes(request.path)) {
                    return h.continue;
                }

                server.log(['info', '***** opendistro-security'], "file:AuthType func:setupAuthScheme obj:unauthenticatedRoutes " + JSON.stringify(this.unauthenticatedRoutes));
                ...
                if (this.unauthenticatedRoutes.includes(request.path)) {
                    ...
                    server.log(['info', '***** opendistro-security'], "file:AuthType func:setupAuthScheme obj:serverUser " + JSON.stringify(credentials));

                    return h.authenticated({credentials});
                };

                try {
                    server.log(['info', '***** opendistro-security'], "file:AuthType func:setupAuthScheme msg:test cookie ");
                    credentials = await this.server.auth.test('security_access_control_cookie', request);
                    return h.authenticated({credentials})
                 } catch(error) {
                    server.log(['info', '***** opendistro-security'], "file:AuthType func:setupAuthScheme obj:error ");
                    ...
                    server.log(['info', '***** opendistro-security'], "file:AuthType func:setupAuthScheme obj:authHeaderCredentials " + JSON.stringify(authHeaderCredentials));

                    if (authHeaderCredentials) {
                        try {
                            let {session} = await request.auth.securitySessionStorage.authenticate(authHeaderCredentials);

                            server.log(['info', '***** opendistro-security'], "file:AuthType func:setupAuthScheme obj:session " + JSON.stringify(session));
                            ...
                            return h.authenticated({
                                ...
                                server.log(['info', '***** opendistro-security'], "file:AuthType func:setupAuthScheme msg:authError ");
                 ...
                 server.log(['info', '***** opendistro-security'], "file:AuthType func:setupAuthScheme msg:onUnAuthenticated ");
    ...
    sessionValidator(server) {
        ...
        server.log(['info', '***** opendistro-security'], "file:AuthType func:sessionValidator");
        ...
        let validate = async(request, session) => {
            ...
            if (session.authType !== this.type) {
                ...
                server.log(['info', '***** opendistro-security'], "file:AuthType func:sessionValidator msg:invalid session");
            ...
            if (differentAuthHeaderCredentials) {
                try {
                    ...
                    server.log(['info', '***** opendistro-security'], "file:AuthType func:sessionValidator obj:authResponse" + JSON.stringify(authResponse));

                    return {valid: true, credentials: authResponse.session};
                } catch(error) {
                    server.log(['info', '***** opendistro-security'], "file:AuthType func:sessionValidator msg:error");
                    request.auth.securitySessionStorage.clearStorage();
                    return {valid: false};
                }
            }
    ...
    async assignAuthHeader(request) {
        ...
        if (! request.headers[this.authHeaderName]) {
            ...
            this.server.log(['info', '***** opendistro-security'], "file:AuthType func:assignAuthHeader obj:authHeaderName " + this.authHeaderName);
            ...
            if (session) {
                ...
                try {
                    ...
                    this.server.log(['info', '***** opendistro-security'], "file:AuthType func:assignAuthHeader obj:sessionValidationResult " + sessionValidationResult.valid);
            ...
            if (session && session.credentials) {
                try {
                    ...
                    this.server.log(['info', '***** opendistro-security'], "file:AuthType func:assignAuthHeader msg:done");
                } catch (error) {
                    this.server.log(['security', 'error'], `An error occurred while computing auth headers, clearing session: ${error}`);
    ...
    registerAssignAuthHeader() {
        this.server.log(['info', '***** opendistro-security'], "file:AuthType func:registerAssignAuthHeader");
        this.server.ext('onPreAuth', (request, h) => {
            try {
                this.assignAuthHeader(request);
            } catch(error) {
                this.server.log(['info', '***** opendistro-security'], "file:AuthType func:registerAssignAuthHeader msg:error");
