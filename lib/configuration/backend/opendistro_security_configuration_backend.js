...
export default class SecurityConfigurationBackend {
   ...
   */
    ...
    async save(headers, resourceName, id, body) {
        ...
        console.log(result)
        console.log("SecurityConfigurationBackend")
    ...
    async saveResource(headers, resourceName, body) {
        const result = Joi.validate(body, this.getValidator(resourceName));
        ...
        const authHeaders = filterAuthHeaders(headers, this._esconfig.requestHeadersWhitelist);
        try {
            const response = await this._client('opendistro_security.saveResourceCurrent', {
                resourceName: resourceName,
                body: body,
                headers: authHeaders
            });
    ...
    getIndexMappings = async ({ headers, payload: { index } }) => {
        try {
          const authHeaders = filterAuthHeaders(headers, this._esconfig.requestHeadersWhitelist);
          const mappings = await this._client('opendistro_security.getIndexMappings', {
            headers: authHeaders,
            index: index.join(','),
