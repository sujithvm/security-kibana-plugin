...
export default function (pluginRoot, server, APP_ROOT, API_ROOT) {
  ...
  server.route({
    ...
    async handler(request, h) {
        try {
          var p = `${API_ROOT}/configuration/{resourceName}/{id}`
          console.log("Path: " + p)
  ...
  server.route({
    ...
    async handler(request, h) {
        try {
          var p = `${API_ROOT}/configuration/{resourceName}`
          console.log("Path: " + p)
          const response = await backend.saveResource(request.headers, request.params.resourceName, request.payload);
          return {
              message: response.message
          }
        } catch (error) {
